...
public class Iap extends Service {
            ...
            try {
              ...
              if (responseCode == BillingResponseCode.OK) {
                ...
                sendPurchasePluginResult(
                  new Payload(Payload.Status.OK, result)
                );
              } else {
                sendPurchasePluginResult(
                  new Payload(Payload.Status.ERROR, responseCode)
                );
              }
            } catch (JSONException e) {
              sendPurchasePluginResult(
                new Payload(Payload.Status.ERROR, e.getMessage())
              );
            }
          }
        }
  ...
  private void setPurchaseUpdatedListener(Callback callbackContext) {
    purchaseUpdated = callbackContext;
  }

  private void consume(String token, Callback callbackContext) {
  ...
  private void startConnection(Callback callbackContext) {
    try {
  ...
  private void purchase(String productIdOrJson, Callback callbackContext) {
    try {
  ...
  private void getPurchases(Callback callbackContext) {
    if (billingClient == null) {
      billingClient = getBillingClient();
      callbackContext.error("Billing client is not connected");
      return;
  ...
  private void sendPurchasePluginResult(Payload result) {
    if (purchaseUpdated != null) {
      result.setKeepCallback(true);
      purchaseUpdated.sendPayload(result);
    }
  }
