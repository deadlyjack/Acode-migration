...
public class BackgroundExecutor extends Service {
    ...
    private final Map<String, Callback> processCallbacks = new ConcurrentHashMap<>();
    ...
    public void initialize(BridgeContext bridgeContext) {
        ...
        this.processManager = new ProcessManager(host.getContext());
    }
    ...
    private void writeToProcess(String pid, String input, Callback callbackContext) {
        try {
    ...
    private void stopProcess(String pid, Callback callbackContext) {
    ...
    private void isProcessRunning(String pid, Callback callbackContext) {
        Process process = processes.get(pid);
    ...
    private void listProcesses(Callback callbackContext) {
    ...
    private void loadLibrary(String path, Callback callbackContext) {
    ...
    private void sendPayload(Callback ctx, String message, boolean keepCallback) {
        Payload result = new Payload(Payload.Status.OK, message);
        result.setKeepCallback(keepCallback);
        ctx.sendPayload(result);
    }

    private void sendPluginMessage(String pid, String message) {
        Callback ctx = processCallbacks.get(pid);
        if (ctx != null) {
            sendPayload(ctx, message, true);
        }
    }
    ...
    private void listAllProcesses(Callback callbackContext) {
        try {
    ...
    private void killProcess(int pid, Callback callbackContext) {
        try {
