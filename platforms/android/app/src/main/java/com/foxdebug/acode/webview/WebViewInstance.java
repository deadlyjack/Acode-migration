...
public class WebViewInstance {
  ...
  void loadURL(String url, final Callback callbackContext) {
    if (isDestroyed) {
  ...
  void loadHTML(final String html, final Callback callbackContext) {
    if (isDestroyed) {
      callbackContext.error("WebView has been destroyed");
      return;
    }
  ...
  void evaluate(String js, final Callback callbackContext) {
    if (isDestroyed) {
      callbackContext.error("WebView has been destroyed");
      return;
    }
  ...
  void postMessage(String message, final Callback callbackContext) {
    if (isDestroyed) {
      callbackContext.error("WebView has been destroyed");
      return;
    }
  ...
  void show(final Callback callbackContext) {
    if (isDestroyed) {
      callbackContext.error("WebView has been destroyed");
      return;
    }
  ...
  void hide(final Callback callbackContext) {
    if (isDestroyed) {
      callbackContext.error("WebView has been destroyed");
      return;
    }
   ...
   * destroy() is called.
   */
  private void hideFullscreen(Callback callbackContext) {
    if (isDestroyed) {
      callbackContext.error("WebView has been destroyed");
      return;
    }
  ...
  void reload(final Callback callbackContext) {
    if (isDestroyed) {
      callbackContext.error("WebView has been destroyed");
      return;
    }
  ...
  void onPageFinished(WebView view) {
    ...
    try {
      ...
      plugin.sendEventToBridge(id, "pageFinished", data);
    } catch (JSONException e) {
  ...
  private class InstanceWebChromeClient extends WebChromeClient {
    ...
    public void onReceivedTitle(WebView view, String pageTitle) {
      ...
      try {
        JSONObject data = new JSONObject();
        data.put("title", pageTitle != null ? pageTitle : "");
        plugin.sendEventToBridge(id, "titleChanged", data);
      } catch (JSONException e) {
  ...
  public class JsBridge {
    ...
    public void postMessage(String message) {
      plugin.sendMessageToBridge(id, message);
    }
  }
}
