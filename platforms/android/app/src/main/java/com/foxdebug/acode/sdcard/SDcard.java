package com.foxdebug.acode.sdcard;
...
public class SDcard extends Service {
  ...
  private Callback activityResultCallback;
  ...
  public void initialize(BridgeContext bridgeContext) {
    ...
    this.context = bridgeContext.getWebActivity();
    this.activity = bridgeContext.getWebActivity();
  ...
  public void openDocumentFile(String mimeType, Callback callback) {
    ...
    host.startActivityForResult(this, intent, this.OPEN_DOCUMENT);
  }

  public void getImage(String mimeType, Callback callback) {
    ...
    intent.setType(mimeType);
    activityResultCallback = callback;
    host.startActivityForResult(this, intent, this.PICK_FROM_GALLERY);
  }

  public void getStorageVolumes(Callback callback) {
    try {
  ...
  public void getStorageAccess(String SDCardUUID, Callback callback) {
    Intent intent = null;
    ...
    activityResultCallback = callback;
    host.startActivityForResult(this, intent, REQUEST_CODE);
  }
  ...
  private void readFile(String filename, Callback callback) {
    host
      .getThreadPool()
      .execute(
        new Runnable() {
  ...
  private void readAsText(final String filename, final String encoding, final Callback callback) {
    host
      .getThreadPool()
      .execute(
        new Runnable() {
  ...
  private void createDir(String parent, String name, Callback callback) {
  ...
  private void delete(String filename, Callback callback) {
  ...
  private void move(String src, String dest, final Callback callback) {
  ...
  private void copy(String src, String dest, final Callback callback) {
    final String splittedStr[] = src.split(SEPARATOR, 2);
  ...
  private void listDir(String src, String parentId, Callback callback) {
    host
      .getThreadPool()
      .execute(
        new Runnable() {
  ...
  private void getStats(String filename, Callback callback) {
    host
      .getThreadPool()
      .execute(
        new Runnable() {
  ...
  private void exists(String path, Callback callback) {
  ...
  private void error(String err, Callback callback) {
    callback.error("ERROR: " + err);
  }

  private void getPath(String uriString, String src, Callback callback) {
    try {
...
class MyFileObserver extends FileObserver {
  ...
  public MyFileObserver(String path, Callback listener) {
    super(path, mask);
    this.listener = listener;
    Log.d("MyFileObserver", "MyFileObserver: " + path);
  }

  public MyFileObserver(File file, Callback listener) {
    super(file, mask);
    this.listener = listener;
  ...
  public void onEvent(int event, String path) {
    ...
    Payload result = new Payload(Payload.Status.OK);
    result.setKeepCallback(true);
    listener.sendPayload(result);
  }
