package com.foxdebug.acode.auth;
...
public class Authenticator extends Service {
    ...
    private volatile Callback loginCallback;

    @Override
    public void serviceInitialize() {
        Log.d(TAG, "Initializing Authenticator Plugin...");
        this.prefManager = new EncryptedPreferenceManager(this.host.getContext(), PREFS_FILENAME);
        ...
        handleAuthCallback(host.getActivity().getIntent());
    }
    ...
    public boolean execute(String action, JSONArray args, Callback callbackContext) throws JSONException {
        ...
        switch (action) {
            case "logout":
                ...
                host.getActivity().runOnUiThread(() -> clearTokenCookie());
            ...
            case "saveToken":
                ...
                host.getActivity().runOnUiThread(() -> setTokenCookie(token));
    ...
    private void startLogin(JSONObject options, Callback callbackContext) {
        ...
        host.getActivity().runOnUiThread(() -> {
            try {
                ...
                customTabsIntent.launchUrl(host.getActivity(), loginUri);
    ...
    private boolean handleAuthCallback(Intent intent) {
        ...
        host.getThreadPool().execute(() -> {
            try {
                ...
                host.getActivity().runOnUiThread(() -> setTokenCookie(token));
                Callback callback = takeLoginCallback();
                if (callback != null) {
                    callback.success();
                }
            } catch (Exception error) {
    ...
    private void setLoginCallback(Callback callbackContext) {
        Callback previousCallback = null;
    ...
    private Callback takeLoginCallback() {
        synchronized (loginCallbackLock) {
