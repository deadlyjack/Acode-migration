package com.foxdebug.acode.webview;
...
public class WebViewPlugin extends Service {
    ...
    try {
      switch (action) {
        case "setMessageCallback":
          ...
          Payload keepResult = new Payload(Payload.Status.NO_RESULT);
          keepResult.setKeepCallback(true);
          callbackContext.sendPayload(keepResult);
          return true;
        case "create":
  ...
  private void create(JSONObject options, final Callback callbackContext) throws JSONException {
      ...
      public void run() {
        ...
        try {
          if (instance.isFullscreen()) {
            ...
            instance.createWebView(host.getActivity());
          }
  ...
  void showFullscreenActivity(String id) {
    Intent intent = new Intent(host.getActivity(), WebViewActivity.class);
  ...
  private void loadURL(String id, String url, Callback callbackContext) {
  ...
  private void loadHTML(String id, String html, Callback callbackContext) {
    WebViewInstance instance = getInstance(id);
    if (instance == null) { callbackContext.error("WebView not found: " + id); return; }
    instance.loadHTML(html, callbackContext);
  }

  private void evaluate(String id, String js, Callback callbackContext) {
    WebViewInstance instance = getInstance(id);
    if (instance == null) { callbackContext.error("WebView not found: " + id); return; }
    instance.evaluate(js, callbackContext);
  }

  private void postMessage(String id, String message, Callback callbackContext) {
    WebViewInstance instance = getInstance(id);
    if (instance == null) { callbackContext.error("WebView not found: " + id); return; }
    instance.postMessage(message, callbackContext);
  }

  private void show(String id, Callback callbackContext) {
    WebViewInstance instance = getInstance(id);
    if (instance == null) { callbackContext.error("WebView not found: " + id); return; }
    instance.show(callbackContext);
  }

  private void hide(String id, Callback callbackContext) {
    WebViewInstance instance = getInstance(id);
    if (instance == null) { callbackContext.error("WebView not found: " + id); return; }
    instance.hide(callbackContext);
  }

  private void reload(String id, Callback callbackContext) {
    WebViewInstance instance = getInstance(id);
    if (instance == null) { callbackContext.error("WebView not found: " + id); return; }
    instance.reload(callbackContext);
  }

  private void destroy(String id, final Callback callbackContext) {
  ...
  public void sendMessageToBridge(String id, String message) {
    ...
    try {
      ...
      Payload result = new Payload(Payload.Status.OK, payload);
      result.setKeepCallback(true);
      messageCallback.sendPayload(result);
    } catch (JSONException e) {
      Log.e(TAG, "Error building message payload", e);
    }
  }

  public void sendEventToBridge(String id, String event, JSONObject data) {
    if (messageCallback == null) return;
